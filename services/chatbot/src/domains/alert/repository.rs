...
impl AlertRepository {
    ...
    pub async fn get_by_id(&self, id: i64) -> anyhow::Result<Option<Alert>> {
        ...
        let sql = "SELECT id, created_at, created_by, chat_id, message_id, username, language_code, lat, lon, description, status, response_text, responded_at, reported_at, ping_at FROM chatbot_alerts WHERE id = ?";
    ...
    pub async fn get_unreported(&self) -> anyhow::Result<Vec<Alert>> {
        let conn = self.db.connect().await?;
        let sql = "SELECT id, created_at, created_by, chat_id, message_id, username, language_code, lat, lon, description, status, response_text, responded_at, reported_at, ping_at FROM chatbot_alerts WHERE status = 'new' AND reported_at IS NULL ORDER BY created_at ASC";
        let mut stmt = conn.prepare(sql).await?;
        let mut rows = stmt
    ...
    pub async fn create(
        ...
        if !force_new {
            let sql = "SELECT id FROM chatbot_alerts 
                       WHERE created_by = ? AND status = 'draft' AND created_at > (unixepoch() - 604800) 
        ...
        let sql = "INSERT INTO chatbot_alerts (created_at, created_by, chat_id, message_id, username, language_code, lat, lon, status, ping_at) 
                   VALUES (unixepoch(), ?, ?, ?, ?, ?, ?, ?, 'draft', unixepoch() + 600)";
    ...
    pub async fn update_location(&self, id: i64, lat: f64, lon: f64) -> anyhow::Result<()> {
        let conn = self.db.connect().await?;
        let sql =
            "UPDATE chatbot_alerts SET lat = ?, lon = ?, ping_at = unixepoch() + 600 WHERE id = ?";
    ...
    pub async fn update_description(&self, id: i64, description: &str) -> anyhow::Result<()> {
        let conn = self.db.connect().await?;
        let sql =
            "UPDATE chatbot_alerts SET description = ?, ping_at = unixepoch() + 600 WHERE id = ?";
        let params = vec![Value::Text(description.to_string()), Value::Integer(id)];
        conn.execute(sql, params_from_iter(params)).await?;
        Ok(())
    }

    pub async fn touch_ping(&self, id: i64) -> anyhow::Result<()> {
        let conn = self.db.connect().await?;
        let sql = "UPDATE chatbot_alerts SET ping_at = unixepoch() + 600 WHERE id = ?";
        conn.execute(sql, params_from_iter(vec![Value::Integer(id)]))
            .await?;
        Ok(())
    }

    pub async fn disable_ping(&self, id: i64) -> anyhow::Result<()> {
        let conn = self.db.connect().await?;
        let sql = "UPDATE chatbot_alerts SET ping_at = NULL WHERE id = ?";
        conn.execute(sql, params_from_iter(vec![Value::Integer(id)]))
            .await?;
        Ok(())
    }

    pub async fn get_pending_pings(&self) -> anyhow::Result<Vec<Alert>> {
        let conn = self.db.connect().await?;
        let sql = "SELECT id, created_at, created_by, chat_id, message_id, username, language_code, lat, lon, description, status, response_text, responded_at, reported_at, ping_at
                   FROM chatbot_alerts
                   WHERE status = 'draft'
                     AND ping_at IS NOT NULL
                     AND ping_at <= unixepoch()
