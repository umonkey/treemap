...
impl TreesBySpeciesReporter {
    ...
    fn get_reports(&self, species: &Vec<String>, trees: &[Tree]) -> Vec<TreesBySpeciesReport> {
    ...
    fn get_report(&self, species: &str, trees: &[Tree]) -> TreesBySpeciesReport {
    ...
    fn filter_by_species(&self, species: &str, trees: &[Tree]) -> Vec<Tree> {
        if species.is_empty() {
            return vec![];
        }
