...
API keys and tokens cannot be stored in the config file. Instead, they are read from the environment variables or files under `/run/secrets` with the same name. The following secrets are supported: FILES_KEY, FILES_SECRET, OSM_CLIENT_SECRET, OSM_TOKEN, TURSO_TOKEN, JWT_SECRET, SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_SECRET, FROM_EMAIL, MAPBOX_TOKEN.
