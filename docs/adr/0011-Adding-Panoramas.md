...
- Backend: we will use internal storage and processing pipelines for hosting imagery and managing metadata. This gives us full control over data processing, storage, and retrieval.
- Viewer: we will use Pannellum as our panoramic viewer. Pannellum is simple, fast, and allows us to exclude unnecessary navigational features that don't fit our specific triangulation workflow.
...
- Controlled UX: using Pannellum ensures a focused interface without unnecessary viewer UI overhead.
- Dependency Management: introducing internal panorama processing and Pannellum (rendering) as core components.
